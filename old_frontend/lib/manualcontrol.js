...
async function ManualControlMove(direction) {
    if (manualControlEnabled) {
        manualControlLoadingBar.setAttribute("indeterminate", "indeterminate");
        try {
            await ApiService.setManualControl(direction);
            //postponeRefreshManualControlMode();
        } catch (err) {
            ons.notification.toast(err.message,
                {buttonLabel: "Dismiss", timeout: window.fn.toastErrorTimeout});
...
async function refreshManualControlMode() {
    try {
        ...
        var StatusStateAttribute = res.find(e => {
            return e.__class === "StatusStateAttribute";
        });
...
window.ManualControlMove = ManualControlMove;
