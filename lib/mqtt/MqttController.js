...
class MqttController {
    ...
    getHassMigrationTopics() {
        return [
            ...
            this.hassController.autoconfPrefix + "/sensor/" + this.identifier + "/GoToLocationCapability/config",
            this.hassController.autoconfPrefix + "/sensor/" + this.identifier + "/WifiConfigurationCapability/config",
            this.hassController.autoconfPrefix + "/sensor/" + this.identifier + "/ZoneCleaningCapability/config",
            this.hassController.autoconfPrefix + "/sensor/" + this.identifier + "/ConsumableStateAttribute_+/config",
        ];
    }
    ...
    async migrateHass(topic) {
        ...
        if (!topic.includes("Consumable")) {
            await this.asyncClient.unsubscribe(topic);
        } else {
            setTimeout(() => {
                if (this.unsubscribedConsumables === undefined) {
                    this.unsubscribedConsumables = true;
                    const topic = this.getHassMigrationTopics().filter(value => value.includes("Consumable"), this)[0];
    ...
    async connect() {
        if (!this.client || (this.client && this.client.connected === false && this.client.reconnecting === false)) {
            ...
            this.client.on("message", (topic, message) => {
                // TODO: temporary, remove!
                if (topic.startsWith(this.hassController.autoconfPrefix + "/")) {
