...
class MqttController {
    ...
    async refresh(handle) {
        ...
        const topic = handle.getBaseTopic();
        const value = await handle.getHomie();
        if (value !== null && this.last_payloads[topic] !== value) {
            try {
                await this.asyncClient.publish(topic, value, {qos: this.qos, retain: handle.retained});
                this.last_payloads[topic] = value;
            } catch (e) {
                Logger.warn("MQTT publication failed, topic " + topic, e);
            }
        }
