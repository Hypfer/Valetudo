...
class MqttController {
                ...
                if (e && e.message === "Not supported") {
                    ...
                    if (this.isInitialized()) {
                        (async () => {
                            ...
                            try {
                                // Try to clean up the internal state and unsubscribe from internal events.
                                Logger.info("Deconfiguring MQTT due to previous error");
                                const deconfOptions = {
                                    cleanHass: false,
                                    cleanHomie: false,
                                    cleanValues: false,
                                ...
                                // This isn't really an issue since we're in an error state.
                                Logger.trace("MQTT deconfiguration failed", err);
                            }
                        })().then();
                    }
                }
    ...
    async publishHass(topic, message, options) {
        ...
        if (lines !== undefined && lines.length > 1 && !lines[1].includes(HassController.name)) {
