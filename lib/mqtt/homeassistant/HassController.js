...
class HassController {
    ...
    async refresh(component, topics) {
        ...
        for (const [topic, payload] of Object.entries(topics)) {
            const topic_to_publish = topic === "" ? base : base + "/" + topic;
            const payload_to_publish =
              payload instanceof String || payload instanceof Buffer ?
                  payload :
                  JSON.stringify(payload);
            if (this.controller.last_payloads[topic_to_publish] !== payload_to_publish) {
                ...
                this.controller.last_payloads[topic_to_publish] = payload_to_publish;
            }
        }
    }
