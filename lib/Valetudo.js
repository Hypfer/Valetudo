...
const Valetudo = function() {
    console.error(GLOBALS.LOCALHOST)
    this.configuration = new Configuration();
    this.address = process.env.VAC_ADDRESS ? process.env.VAC_ADDRESS : GLOBALS.LOCALHOST;
    ...
    this.webPort = process.env.VAC_WEBPORT ? parseInt(process.env.VAC_WEBPORT) : GLOBALS.DEFAULT_PORT;
    this.map = new MapDTO({
...
function readValueFromDeviceConf(key) {
    let deviceConf;
    try {
        deviceConf = fs.readFileSync(GLOBALS.DEFAULT_DEVICE_CONF);
    } catch(e) {
        console.error(e);
    }
...
Valetudo.NATIVE_TOKEN_PROVIDER = function() {
    const token = fs.readFileSync(GLOBALS.DEFAULT_DEVICE_TOKEN);
...
Valetudo.CLOUD_KEY_PROVIDER = function() {
    if(process.env.VAC_CLOUDKEY) {
        ...
        // noinspection JSConstructorReturnsPrimitive
        return cloudKey ? cloudKey : GLOBALS.DEFAULT_CLOUD_KEY; //This doesnt work but it wont crash the system
    }
};
