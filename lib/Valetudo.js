...
class Valetudo {
    constructor() {
        ...
        if (MODEL_TO_IMPLEMENTATION[this.model.name]) {
            this.vacuum = new MODEL_TO_IMPLEMENTATION[this.model.name](robotArgs);
        } else {
            throw new Error("No implementation found for " + this.model.name);
        }
