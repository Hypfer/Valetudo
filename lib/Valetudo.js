...
Valetudo.MODEL_PROVIDER = function() { //TODO: merge with CLOUD_KEY_PROVIDER
    if(process.env.VAC_MODEL) {
        ...
        if(deviceConf) {
            const model = deviceConf.toString().match(/^model=(.*)$/m);

            if(Array.isArray(model) && model[1]) {
                // noinspection JSConstructorReturnsPrimitive
                return model[1];
            }
        }
        ...
        // noinspection JSConstructorReturnsPrimitive
        return "unknown"; //This doesnt work but it wont crash the system
    }
};
