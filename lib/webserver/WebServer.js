...
const WebServer = function (options) {
    ...
    this.app.get("/api/remote/map", function (req, res) {
        ...
        WebServer.FIND_LATEST_MAP(function (err, data) {
                    ...
                    if (!err) {
                        ...
                        fs.readdirSync(tmpDir + directory)
                            ...
                            .forEach(function (file) {
                                numerOfFiles++;
                                if (numerOfFiles > 5) {
                                    fs.unlink(tmpDir + directory + file, err => {
            ...
            self.configuration.get("dontFlipGridMap"), //TODO: this whole dontFlipGridMap thing is just weird. Fix this mess
            self.configuration.get("preferGridMap") //TODO: same for preferGridMap
        );
    });
    ...
    this.app.use(express.static((process.env.VAC_TMP_PATH ? process.env.VAC_TMP_PATH : "/tmp"))); //TODO: Don't. No. This is bad.
...
WebServer.PARSE_GRID_MAP = function (buf, dontFlipGridMap) {
    const map = [];
...
//TODO: why is nothing using this???
WebServer.PARSE_MAP_AUTO = function (filename, dontFlipGridMap) {
    ...
    try {
        ...
        if (mapBytes.length === WebServer.CORRECT_GRID_MAP_FILE_SIZE) {
            mapData = WebServer.PARSE_GRID_MAP(mapBytes, dontFlipGridMap);
...
WebServer.FIND_LATEST_MAP = function (callback, dontFlipGridMap, preferGridMap) {
    if (process.env.VAC_MAP_TEST) {
        ...
        WebServer.FIND_LATEST_MAP_IN_RAMDISK(callback, dontFlipGridMap, preferGridMap);
    }
};

WebServer.FIND_LATEST_MAP_IN_RAMDISK = function (callback, dontFlipGridMap, preferGridMap) {
                            ...
                            let loadGridMap = function () {
                                fs.readFile("/dev/shm/GridMap", function (err, gridMapFile) {
                                    if (err) {
                                        ...
                                        callback(null, {
                                            mapData: WebServer.PARSE_GRID_MAP(gridMapFile, dontFlipGridMap),
                                            log: log
                                        })
                                    }
                                })
