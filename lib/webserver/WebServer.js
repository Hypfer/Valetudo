...
const WebServer = function (options) {
    ...
    this.app.get("/api/current_status", function(req, res) {
        self.vacuum.getCurrentStatus().then((data) => res.json(data),
        ...
        fs.readFile("/etc/os-release", function (err, data) {
            if (err) {
                ...
                if (extractedOsRelease) {
                    ...
                    var packageContent = fs.readFileSync(rootDirectory + "/package.json", {"encoding": "utf-8"});
    ...
    this.app.put("/api/start_cleaning", function (req, res) {
        self.vacuum.getCurrentStatus().then(
            data => {
    ...
    this.app.put("/api/set_ssh_keys", function (req, res) {
        ...
        if (req.body && req.body.keys && typeof req.body.keys === "string") {
            self.sshManager.setSSHKeys(req.body.keys)
