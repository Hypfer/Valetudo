...
const LOCAL_UPLOAD_FOLDER = '/mnt/data/valetudo/uploads';
const upload = multer({ dest: LOCAL_UPLOAD_FOLDER });
...
const WebServer = function (options) {
    ...
    this.uploadLocation = LOCAL_UPLOAD_FOLDER;
            ...
            req.on('end', function() {
                ...
                zlib.gunzip(uploadedGzippedMapData, (err, data) => {
                    ...
                    if(hashOfNewMap !== self.map.hash) {
                        if (!err) {
                            const parsedMap = self.map.PARSE(data);
