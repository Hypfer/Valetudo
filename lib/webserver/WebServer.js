...
const WebServer = function (options) {
    ...
    this.app.put("/api/set_ssh_keys", function (req, res) {
        if (!self.configuration.get("allowSSHKeyUpload")) return res.status(403).send("ssh key management is disabled");
    ...
    let firmware_md5 = null;
    this.app.post("/api/upload_firmware_md5", function(req, res) {
        if (req.body && req.body.md5 && typeof req.body.md5 === "string") {
            firmware_md5 = req.body.md5;
            res.end("success");
        } else {
            res.status(400).send("Invalid request");
        }
    ...
    this.app.post("/api/upload_firmware_update", function(req, res) {
        if (!firmware_md5) {
            return res.status(400).send("call '/api/upload_firmware_md5' first");
        }
        ...
        const server2 = http.createServer((req2, res2) => {
            if (req2.headers["range"]) {
                res2.writeHead(400, { 'Content-Type': 'text/plain' });
                res2.end("ranges are not supported");
            }
            res.write("download started");
            res2.writeHead(200, { 'Content-Type': 'application/octet-stream', 'Content-Length': length });
            req.pipe(res2);
        });
        ...
        self.vacuum.update("http://127.0.0.1:9091/", firmware_md5, function() {
            res.write("sent ota request");
        });
        firmware_md5 = null;
    }).listen(9090);

    this.app.get("/api/update_status", function(req, res) {
        self.vacuum.getUpdateStatus(function (err, data) {
            if (err) {
                res.status(500).send(err.toString());
            } else {
                res.json(data);
    ...
    this.app.get("/api/update_progress", function(req, res) {
        self.vacuum.getUpdateProgress(function (err, data) {
            if (err) {
                res.status(500).send(err.toString());
            } else {
                res.json(data);
