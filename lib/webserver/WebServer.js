 ...
 * @param options.vacuum {Roborock} (actually a MiioDevice, but that's not complete yet)
...
const WebServer = function (options) {
    ...
    this.app.put("/api/miio/map_upload_handler", function(req, res) {
        console.debug('map_upload_handler');
        if(!self.mapUploadInProgress) {
            ...
            req.on('end', function() {
                ...
                const inflate = this.model.viomiApi ? zlib.inflate : zlib.gunzip;

                inflate(uploadedGzippedMapData, (err, data) => {
                    ...
                    if(hashOfNewMap !== self.map.hash) {
                        if (!err) {
                            const parsedMap = this.model.viomiApi ? new ViomiMapParser(data).parse()
                                                                  : RRMapParser.PARSE(data);
    ...
    */
    ...
    this.app.get("/api/token", function(req, res) {
        res.json({token: self.dummycloud.CLOUD_KEY_PROVIDER.token.toString("hex")});
    });
