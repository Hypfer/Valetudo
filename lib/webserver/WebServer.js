...
const WebServer = function (options) {
    ...
    this.app.put("/api/list_maps", function (req, res) {
    ...
    this.app.put("/api/load_map", function (req, res) {
        if (req.body && req.body.name !== undefined) {
            MapManager.loadMap(req.body.name, function (err, data) {
                if (err) {
                    res.status(500).send(err.toString());
                } else {
                    res.json({message: "ok"});
    ...
    this.app.get("/api/get_multifloor", function (req, res) {
        res.json({...self.configuration.get("multifloor")});
    });

    this.app.put("/api/set_multifloor", function (req, res) {
        if(req.body && typeof req.body === "object" && typeof req.body.enabled === "boolean") {
            self.configuration.set("multifloor", {
