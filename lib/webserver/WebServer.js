...
const WebServer = function (options) {
    ...
    this.app.get("/api/get_telegram_config", function (req, res) {
        if(self.configuration.get("telegram_token")){
            token = self.configuration.get("telegram_token");
            telegram_send_consumables = self.configuration.get("telegram_send_consumables");
            telegram_send_consumables_every = self.configuration.get("telegram_send_consumables_every");
        } else {
            token = "";
            telegram_send_consumables = true;
            telegram_send_consumables_every = false;
        }
        res.json({
            telegram_token: token,
            telegram_send_consumables: telegram_send_consumables,
            telegram_send_consumables_every: telegram_send_consumables_every
        });
    });
    this.app.get("/api/get_telegram_status", function (req, res) {
        res.json({
            running: self.telegramClient.get_status()
        });
    });
    
    this.app.put("/api/set_telegram_config", function (req, res) {
        self.configuration.set("telegram_token", req.body.telegram_token);
        self.configuration.set("telegram_send_consumables", req.body.telegram_send_consumables);
        self.configuration.set("telegram_send_consumables_every", req.body.telegram_send_consumables_every);
        if(req.body.telegram_token) {
            self.telegramClient.initiate({
                vacuum: self.vacuum,
                telegramtoken: req.body.telegram_token,
                sendconsumables: req.body.telegram_send_consumables,
                sendconsumablesevery: req.body.telegram_send_consumables_every
            });
        } else {
            self.telegramClient.initiate({
                vacuum: self.vacuum,
