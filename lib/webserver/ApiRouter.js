...
const ApiRouter = function (webserver) {
    ...
    router.put("/start_cleaning_segments", async (req, res) => {
        try {
            if (req.body) {
                await vacuum.startCleaningSegments(req.body);
                res.sendStatus(200);
            } else {
                res.status(400).send("segments missing");
            }
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/set_ssh_keys", async (req, res) => {
        try {
            if (!configuration.get("allowSSHKeyUpload")) {
    ...
    router.get("/map/latest", async (req, res) => {
        res.json(vacuum.robotState.map);
    });
