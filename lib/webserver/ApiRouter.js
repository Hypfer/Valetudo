...
const ApiRouter = function(webserver) {
    ...
    router.post("/timers", async (req, res) => {
        try {
            if (req.body && req.body.cron) {
                let data = await vacuum.addTimer(req.body.cron);
                res.json(data);
            } else {
    ...
    router.put("/timers/:timerID", async (req, res) => {
        try {
            if (req.body && req.body.enabled !== undefined) {
                let data = await vacuum.toggleTimer(req.params.timerID, req.body.enabled);
                res.json(data);
            } else {
                res.status(400).send("Invalid request");
            }
    ...
    router.delete("/timers/:timerID", async (req, res) => {
        try {
            let data = await vacuum.deleteTimer(req.params.timerID);
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.post("/set_dnd", async (req, res) => {
        try {
            if (req.body && req.body.start_hour && req.body.start_minute && req.body.end_hour && req.body.end_minute) {
                let data = await vacuum.setDndTimer(req.body.start_hour, req.body.start_minute, req.body.end_hour, req.body.end_minute);
                res.json(data);
            } else {
                res.status(400).send("invalid request");
            }
    ...
    router.put("/delete_dnd", async (req, res) => {
        try {
            let data = await vacuum.deleteDndTimer();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.post("/set_timezone", async (req, res) => {
        try {
            if (req.body && req.body.new_zone) {
                let data = await vacuum.setTimezone(req.body.new_zone);
                res.json(data);
            } else {
                res.status(400).send("invalid request");
            }
    ...
    router.put("/clean_record", async (req, res) => {
        try {
            if (req.body && req.body.recordId) {
                let data = await vacuum.getCleanRecord(req.body.recordId);
    ...
    router.put("/start_cleaning", async (req, res) => {
        try {
            ...
            if (data.in_cleaning === 2 && data.state === "PAUSED") {
                data = await vacuum.resumeCleaningZone();
            } else {
                data = await vacuum.startCleaning();
            }
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
    ...
    router.put("/pause_cleaning", async (req, res) => {
        try {
            let data = await vacuum.pauseCleaning();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/stop_cleaning", async (req, res) => {
        try {
            let data = await vacuum.stopCleaning();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/go_to", async (req, res) => {
        try {
            if (req.body && req.body.x !== undefined && req.body.y !== undefined) {
                let data = await vacuum.goTo(req.body.x, req.body.y);
                res.json(data);
            } else {
    ...
    router.put("/start_cleaning_zone_by_coords", async (req, res) => {
        try {
            if (req.body) {
                let data = await vacuum.startCleaningZoneByCoords(req.body);
                res.json(data);
            } else {
                res.status(400).send("coordinates missing");
            }
    ...
    router.put("/start_cleaning_zones_by_id", async (req, res) => {
        try {
            if (req.body) {
                let data = await vacuum.startCleaningZonesById(req.body);
                res.json(data);
            } else {
                res.status(400).send("coordinates missing");
            }
    ...
    router.put("/fanspeed", async (req, res) => {
        try {
            if (req.body && req.body.speed && req.body.speed <= 105 && req.body.speed >= 0) {
                let data = await vacuum.setFanSpeed(req.body.speed);
                res.json(data);
            } else {
    ...
    router.put("/set_sound_volume", async (req, res) => {
        try {
            if (req.body && req.body.volume && req.body.volume <= 100 && req.body.volume >= 0) {
                let data = await vacuum.setSoundVolume(req.body.volume);
                res.json(data);
            } else {
    ...
    router.put("/test_sound_volume", async (req, res) => {
        try {
            let data = await vacuum.testSoundVolume();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/wifi_configuration", async (req, res) => {
        try {
            if (req.body && req.body.ssid && req.body.password) {
                let data = await vacuum.configureWifi(req.body.ssid, req.body.password);
                res.json(data);
            } else {
    ...
    router.put("/reset_consumable", async (req, res) => {
        try {
            if (req.body && typeof req.body.consumable === "string") {
                let data = await vacuum.resetConsumable(req.body.consumable);
                res.json(data);
            } else {
                res.status(400).send("Invalid request");
            }
    ...
    router.put("/find_robot", async (req, res) => {
        try {
            let data = await vacuum.findRobot();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/drive_home", async (req, res) => {
        try {
            let data = await vacuum.driveHome();
            res.json(data[0]);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/spot_clean", async (req, res) => {
        try {
            let data = await vacuum.spotClean();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/start_manual_control", async (req, res) => {
        try {
            let data = await vacuum.startManualControl();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/stop_manual_control", async (req, res) => {
        try {
            let data = await vacuum.stopManualControl();
            res.json(data);
        } catch (err) {
            res.status(500).send(err.toString());
        }
    ...
    router.put("/set_manual_control", async (req, res) => {
        try {
            if (req.body && req.body.angle !== undefined && req.body.velocity !== undefined
                ...
                let data = vacuum.setManualControl(req.body.angle, req.body.velocity, req.body.duration, req.body.sequenceId);
                res.json(data);
            } else {
                res.status(400).send("Invalid request");
            }
    ...
    router.put("/set_carpet_mode", async (req, res) => {
        try {
            if (req.body && req.body.enable !== undefined && req.body.current_integral !== undefined && req.body.current_low !== undefined
                ...
                let data = await vacuum.setCarpetMode(req.body.enable, req.body.current_integral, req.body.current_low, req.body.current_high, req.body.stall_time);
                res.json(data);
            } else {
                res.status(400).send("Invalid request");
            }
    ...
    router.post("/install_voice_pack", upload.single("file"), async (req, res) => {
        if (req.file){
            ...
            //Remove old uploads
            for (const file of fs.readdirSync(webserver.uploadLocation)) {
                if (file.endsWith(".pkg"))
                    fs.unlink(path.join(webserver.uploadLocation, file), rmerr => {});
            }

            var tmpname = path.join(webserver.uploadLocation, path.basename(req.file.path) + ".pkg");
