...
ViomiMapParser.prototype.parse = function() {
    ...
    if (historyMask & 4) {
        ...
        for (let i = 0; i < head[1]; i++) {
            ...
            this.history.push([
                (20 + this.buf.readFloatLE(this.offset + 1)) * 1000,
                (20 + this.buf.readFloatLE(this.offset + 5)) * 1000,
                this.buf.readUInt8(
                    this.offset) // last byte may be angle or whether robot is in taxi mode/cleaning
            ]);
            this.offset += 9;
        }
    }
    ...
    if (historyMask & 8) {
        ...
        this.chargeStation = this.take(16, 'charge station');
    }

    if (historyMask & 0x10) {
        let head = asInts(this.take(12, 'virtual wall'));
        console.log('VirtualWall', head);
        this.take(92 * head[2], 'virtual wall content');
    }

    if (historyMask & 0x20) {
        let head = asInts(this.take(12, 'area head'));
        console.log('AreaClean', head);
        this.take(92 * head[2], 'area content');
    }

    if (historyMask & 0x40) {
        this.navigateTarget = this.take(20, 'navigate');
    }

    if (historyMask & 0x80) {
        this.realtimePose = this.take(21, 'realtime');
    }
    ...
    return {image: this.img, path: {points: this.points.length ? this.points : this.history}};
};

/** Debug log name and binary location. */
ViomiMapParser.prototype.log = function(name, extra) {
    console.debug(name, (this.offset).toString(16), extra);
};
...
ViomiMapParser.prototype.parsePose = function() {
    ...
    for (let i = 0; i < length; i++) {
        ...
        for (let j = 0; j < elements; j++) {
            pose.push([
                this.buf.readInt16LE(this.offset) * 50, this.buf.readInt16LE(this.offset + 2) * 50,
...
ViomiMapParser.prototype.parseRooms = function() {
    ...
    for (let i = 0; i < length; i++) {
        ...
        this.log('room' + id, this.offset);
        rooms[id] = {
            name: this.buf.toString('utf8', this.offset + 3, this.offset + 3 + nameLen - 1)
        };
...
ViomiMapParser.prototype.parseImg = function() {
    ...
    if (height > 0 && width > 0) {
        ...
        for (let i = 0; i < height * width; i++) {
            ...
            if (val !== 0) {
                coords = [i % width, Math.floor(i / width)]; // height - 1 - Math.floor(i / width)];
