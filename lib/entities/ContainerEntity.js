...
class ContainerEntity extends SerializableEntity {
    ...
    removeMatchingAttributes(options) {
        ...
        needles.forEach(e => Logger.debug("Removing Attribute:", e.toString()));
    ...
    upsertFirstMatchingAttribute(newAttribute) {
        ...
        const newString = newAttribute.toString();
        if (index === -1) {
            Logger.debug("Adding Attribute:", newString);
            this.attributes.push(newAttribute);
        } else {
            if (this.attributes[index].toString() !== newString) {
                Logger.debug("Updating Attribute:", newString);
            }
