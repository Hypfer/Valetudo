...
class ContainerEntity extends SerializableEntity {
    ...
    getAttributeSubscribersMetas(matcher, options) {
        ...
        return this.subscribers.filter((item) => {
            ...
            if (((!exact && matcher.attributeType !== undefined) || exact) && item.matcher.attributeType !== matcher.attributeType) {
                return false;
            }
            // noinspection RedundantIfStatementJS // screw the linter, readability first
            if (((!exact && matcher.attributeSubType !== undefined) || exact) && item.matcher.attributeSubType !== matcher.attributeSubType) {
                return false;
            }
            return true;
        });
