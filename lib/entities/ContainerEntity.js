...
class ContainerEntity extends SerializableEntity {
    ...
    getAttributeSubscribersMetas(matcher, options) {
        ...
        return this.subscribers.filter((item) => {
            if (item.matcher.attributeClass !== matcher.attributeClass) {
                return false;
            }
            if (((!exact && item.matcher.attributeType !== undefined) || exact) && item.matcher.attributeType !== matcher.attributeType) {
                return false;
            }
            // noinspection RedundantIfStatementJS // screw the linter, readability first
            if (((!exact && item.matcher.attributeSubType !== undefined) || exact) && item.matcher.attributeSubType !== matcher.attributeSubType) {
                return false;
            }
            return true;
        });
    ...
    subscribe(subscriber, matcher,) {
        ...
        if (metas.length > 0) {
            meta = metas[0];
            if (metas.length > 1) {
                Logger.error("BUG! Found multiple attribute subscribers metas matching exactly. Please report this issue.");
            }
        } else {
            meta = {
                matcher: matcher,
     ...
     * @param {import("./AttributeSubscriber")} subscriber
     */
    // A method to unsubscribe only from one matcher can technically be added, but I can't imagine a use for it.
    // Unsubscribing from all is what we usually need and it has lower algorithmic complexity when done in bulk this way
    unsubscribeAll(subscriber) {
        const metas = this.subscribers.filter(item => item.subscribers.includes(subscriber));
        const toDelete = [];
        for (const meta of metas) {
            const index = meta.subscribers.indexOf(subscriber);
