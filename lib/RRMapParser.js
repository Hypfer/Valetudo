...
RRMapParser.PARSE_BLOCK = function parseBlock(buf, offset, result) {
    ...
    switch (type) {
        ...
        case RRMapParser.TYPES.PATH:
            ...
            if (points.length > 2) {
                //TODO: I'm unsure about the 90°
                result[type].current_angle = 90 + Math.atan2(points[points.length - 1][1] - points[points.length - 2][1], points[points.length - 1][0] - points[points.length - 2][0]) * 180 / Math.PI;
            }
