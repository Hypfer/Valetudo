...
class RRMapParser {
    ...
    static PARSE_IMAGE_BLOCK(block) {
        ...
        if (parsedBlock.dimensions.height > 0 && parsedBlock.dimensions.width > 0) {
                    ...
                    switch (type) {
                        ...
                        default: {
                            ...
                            if (mayContainSegments) {
                                ...
                                if (segmentId !== 0) {
                                    //We want the outer dimensions so that we can find the center of the segment
                                    ...
                                    coords.forEach((coord, i) => {
                                        const dimension = i === 0 ? "x" : "y";

                                        if (
                                            !parsedBlock.segments[segmentId].dimensions[dimension].min ||
                                            coord < parsedBlock.segments[segmentId].dimensions[dimension].min
                                        ) {
                                            parsedBlock.segments[segmentId].dimensions[dimension].min = coord;
                                        }

                                        if (
                                            !parsedBlock.segments[segmentId].dimensions[dimension].max ||
                                            coord > parsedBlock.segments[segmentId].dimensions[dimension].max
                                        ) {
                                            parsedBlock.segments[segmentId].dimensions[dimension].max = coord;
                                        }
                                    });


                                    parsedBlock.segments[segmentId].pixels.push(coords);
                                }
                            }
            ...
            Object.keys(parsedBlock.segments).filter(k => k !== "count").forEach(k => {
                Object.keys(parsedBlock.segments[k].dimensions).forEach(dK => {
                    parsedBlock.segments[k].dimensions[dK].mid = Math.round((
                        parsedBlock.segments[k].dimensions[dK].max +
                        parsedBlock.segments[k].dimensions[dK].min
                    ) / 2);
                });
            });
