    ...
    static PARSE_IMAGE_BLOCK(block) {
        ...
        if (parsedBlock.dimensions.height > 0 && parsedBlock.dimensions.width > 0) {
            ...
            for (let i = 0; i < block.data_length; i++) {
                ...
                if (val !== 0) {
                    const coords = [
                        (i % parsedBlock.dimensions.width) + parsedBlock.position.left,
                        (parsedBlock.dimensions.height-1 - Math.floor(i / parsedBlock.dimensions.width)) + parsedBlock.position.top
                    ];
                    ...
                    switch (type) {
                        case 0:
                            break;
                        case 1:
                            imageData.obstacle_strong.push(coords[0], coords[1]);
                            break;
                        default: {
                            imageData.floor.push(coords[0], coords[1]);
                            ...
                            if (mayContainSegments) {
                                ...
                                if (segmentId !== 0) {
                                    ...
                                    parsedBlock.segments[segmentId].push(coords[0], coords[1]);
                                }
                            }
