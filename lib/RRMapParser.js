...
class RRMapParser {
    ...
    static PARSE_IMAGE_BLOCK(block) {
        let g3offset = block.header_length > 24 ? 4 : 0;

        const parsedBlock = { //Metadata
            position: {
                top: block.view.readInt32LE(0x08 + g3offset),
                left: block.view.readInt32LE(0x0c + g3offset)
            },
            dimensions: {
                height: block.view.readInt32LE(0x10 + g3offset),
                width: block.view.readInt32LE(0x14 + g3offset)
            },
            pixels: {}
        };
        ...
        if (parsedBlock.dimensions.height > 0 && parsedBlock.dimensions.width > 0) {
            ...
            for (let i = 0; i < block.data_length; i++) {
                const val = block.view[0x18 + g3offset + i];
