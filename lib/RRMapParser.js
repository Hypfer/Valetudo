...
RRMapParser.PARSE = function parse(mapBuf) {
    if (mapBuf[0x00] === 0x72 && mapBuf[0x01] === 0x72) {// rr
        ...
        if (blocks[RRMapParser.TYPES.IMAGE]) { //We need the image to flip everything else correctly
            ...
            parsedMapData.image.xBorderCoordinatesInMM = { //TODO: This name is terrible
                left: parsedMapData.image.position.left * 50
            };
            parsedMapData.image.xBorderCoordinatesInMM.right = parsedMapData.image.xBorderCoordinatesInMM.left + parsedMapData.image.dimensions.width * 50;
            ...
            if (blocks[RRMapParser.TYPES.PATH]) {
                ...
                parsedMapData.path.points = parsedMapData.path.points.map(point => {
                    point[0] = parsedMapData.image.xBorderCoordinatesInMM.left + (parsedMapData.image.xBorderCoordinatesInMM.right - point[0]);
                    return point;
                });

                if (parsedMapData.path.points.length >= 2) {
                    parsedMapData.path.current_angle = 90 +
                        Math.atan2(
            ...
            if (blocks[RRMapParser.TYPES.CHARGER_LOCATION]) {
                ...
                parsedMapData.charger[0] = parsedMapData.image.xBorderCoordinatesInMM.left +
                                            (parsedMapData.image.xBorderCoordinatesInMM.right - parsedMapData.charger[0]);
            }

            if (blocks[RRMapParser.TYPES.ROBOT_POSITION]) {
                parsedMapData.robot = blocks[RRMapParser.TYPES.ROBOT_POSITION].position;
                parsedMapData.robot[0] = parsedMapData.image.xBorderCoordinatesInMM.left +
                                            (parsedMapData.image.xBorderCoordinatesInMM.right - parsedMapData.robot[0]);
            }
