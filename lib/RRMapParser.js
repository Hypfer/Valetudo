...
RRMapParser.PARSE_BLOCK = function parseBlock(buf, offset, result) {
    ...
    switch (type) {
        ...
        case RRMapParser.TYPES.CURRENTLY_CLEANED_ZONES:
            ...
            break;
        case RRMapParser.TYPES.NO_GO_AREAS:
    ...
    if (mapBuf[0x00] === 0x72 && mapBuf[0x01] === 0x72) {// rr
        ...
        if (blocks[RRMapParser.TYPES.IMAGE]) { //We need the image to flip everything else correctly
            ...
            if (blocks[RRMapParser.TYPES.CHARGER_LOCATION]) {
                ...
                parsedMapData.charger[1] = Tools.DIMENSION_MM - parsedMapData.charger[1] - 150;
                //I actually have no idea why it is 15cm off but it is 15cm off
            }
            ...
            if(blocks[RRMapParser.TYPES.NO_GO_AREAS]) {
                parsedMapData.no_go_areas = blocks[RRMapParser.TYPES.NO_GO_AREAS];
