...
class Viomi extends MiioVacuum {
    ...
    enableMopMode(enable) {
        ...
        return this.sendLocal("set_mop", [mopMode]);
    }
    ...
    async goTo(x_coord, y_coord) {
        ...
        await this.sendLocal("set_pointclean", [this.mode, x_coord, y_coord], {});
    }
    ...
    async startCleaningZonesById(zone_ids) {
        ...
        if (zone_ids_from_map.length) {
            ...
            let mode = Mode.ZONED_CLEAN;
            await this.startOperation([mode, 1]);
        } else {
    ...
    async setFanSpeed(speed) {
        ...
        if (suction !== undefined) {
            await this.sendLocal("set_suction", [parseInt(suction, 10)]);
            return await this.pollStatus();
        } else if (typeof speed === "string") {
            if (FAN_GRADES[speed] !== undefined) {
                await this.sendLocal("set_suction", [FAN_GRADES[speed]]);
