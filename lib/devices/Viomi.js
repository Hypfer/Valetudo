...
class Viomi extends MiioVacuum {
    ...
    */
    ...
    updateStatus(newState) {
        ...
        if ("mode" in newState) {
            this.status.mode = newState["mode"];
        }
    ...
    async startCleaningZonesById(zone_ids) {
        ...
        if (zone_ids_from_map.length) {
            ...
            return await this.startOperation(args.concat(zone_ids_from_map));
            ...
            mode = 3; // zoned clean
            return await this.startOperation([mode, 1]);
        } else {
    ...
    stopCleaning() {
        return this.startOperation([this.status.mode, 0]);
    }
}
