...
/** @enum {number} */
const MopMode = Object.freeze({ //TODO: OperationModeStateAttribute
    VACUUM: 0,
    MOP: 2,
    MIXED: 1,
});
...
/** @enum {number} */
const BoxType = Object.freeze({  //TODO: AttachmentStateAttribute
    NONE: 0,
    VACUUM: 1,
    WATER: 2,
    VACUUM_AND_WATER: 3
...
/** @enum {number} */
const Mode = Object.freeze({ //TODO: MovementModeStateAttribute
    NORMAL_CLEANING: 0,
...
class Viomi extends MiioVacuum {
    ...
    parseStatus(newState) {
        if ("run_state" in newState) {
            const oldStateFastPull = MiioVacuum.FAST_MAP_PULLING_STATES.indexOf(this.status.state); //TODO
            this.status.state = STATUS_MAP[newState["run_state"]];
            this.status.human_state = this.status.state;
            const newStateFastPull = MiioVacuum.FAST_MAP_PULLING_STATES.indexOf(this.status.state); //TODO
    ...
    /** @override */
    onStatusChange(changedProperties) { //TODO broken for now
