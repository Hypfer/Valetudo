...
class MiioVacuum {
     ...
     *
     * @protected
     * @param {any} newState json object as received from the robot.
     */
    updateStatus(newState) {
        // Compute the diff for logging purposes.
        const diff = {};
        for (let k in newState) {
            if (this.statusForLogging[k] !== newState[k]) {
                diff[k] = newState[k];
                this.statusForLogging[k] = newState[k];
            }
        }
