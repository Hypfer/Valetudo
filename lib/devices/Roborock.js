...
class Roborock extends MiioVacuum {
    ...
    detectFanSpeeds(msg_ver) {
        let fanSpeeds = {
            "whisper": {label: "Whisper", value: 1},
            "min": {label: "Quiet", value: 38},
            "medium": {label: "Balanced", value: 60},
            "high": {label: "Turbo", value: 75},
            "max": {label: "Max", value: 100},
            "mop": {label: "Mop", value: 105},
        };

        if (msg_ver >= 3) {
            delete fanSpeeds.whisper;
            fanSpeeds.min.value = 101;
            fanSpeeds.medium.value = 102;
            fanSpeeds.high.value = 103;
            fanSpeeds.max.value = 104;
            fanSpeeds.mop.value = 105;
        }

        if (this.model.name === "rockrobo.vacuum.v1") {
            delete fanSpeeds.mop;
        }
