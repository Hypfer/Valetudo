...
Roborock.prototype = Object.create(MiioDevice.prototype);
...
Roborock.prototype.onMessage = function (msg) {
    switch (msg.method) {
        ...
        case "_sync.batch_gen_room_up_url":
            let mapUploadUrls = [];
            for(let i = 0; i < 4; i++) {
                mapUploadUrls.push(this.mapUploadHost + "/api/miio/map_upload_handler?" + process.hrtime())
            }
...
Roborock.prototype.pollMap = function (callback) {
    this.sendCloud({ 'method': 'get_map_v1' }, callback);
};
...
Roborock.GET_ERROR_CODE_DESCRIPTION = function(errorCodeId) {
    if (Roborock.ERROR_CODES.hasOwnProperty(errorCodeId)) {
 ...
 * @param callback
 */
Roborock.prototype.driveHome = function(callback) {
    this.sendLocal("set_charge", [1], {}, Roborock.GET_ARRAY_HANDLER(callback));
};
...
Roborock.prototype.setManualControl = function(angle, velocity, duration, sequenceId, callback) {
    this.sendLocal("app_rc_move", [{"omega": angle, "velocity": velocity, "seqnum": sequenceId, "duration": duration}], {}, callback)
};
...
Roborock.prototype.configureWifi = function(ssid, password, callback) {
    this.sendLocal("miIO.config_router", {"ssid": ssid, "passwd": password, "uid": 0}, {}, callback)
};
...
Roborock.prototype.getCurrentStatus = function(callback) {
    this.sendLocal("get_status", {}, (err, res) => {
        if (res) {
...
Roborock.prototype.savePersistentData = function(persistantData, callback) { //TODO: Store in valetudo config
    if(Array.isArray(persistantData)) {
        ...
        this.sendLocal("save_map", flippedYCoordinates, {}, (err) => { this.pollMap(() => {}); });
    }
