    ...
    async updateAttributesTopic() {
        ...
        try {
            if (this.client && this.client.connected === true) {
                ...
                response.mainBrush = (Math.max(0, 300 - (consumableStatus.main_brush_work_time / 60 / 60))).toFixed(1);
                response.sideBrush = (Math.max(0, 200 - (consumableStatus.side_brush_work_time / 60 / 60))).toFixed(1);
                response.filter = (Math.max(0, 150 - (consumableStatus.filter_work_time / 60 / 60))).toFixed(1);
                response.sensor = (Math.max(0, 30 - (consumableStatus.sensor_dirty_time / 60 / 60))).toFixed(1);

                let cleanSummary = await this.vacuum.getCleanSummary();
                response.cleanTime = (cleanSummary[0] / 60 / 60).toFixed(1);
                response.cleanArea = (cleanSummary[1] / 1000000).toFixed(1);
                response.cleanCount = cleanSummary[2];
                let last_runs = cleanSummary[3];
                if (last_runs.length > 0) {
                    try {
                        let cleanRecord = await this.vacuum.getCleanRecord(parseInt(last_runs[0]));

                        this.last_run_stats = {
                            startTime: cleanRecord[0][0] * 1000, //convert to ms
                            endTime: cleanRecord[0][1] * 1000, //convert to ms
                            duration: cleanRecord[0][2],
                            area: (cleanRecord[0][3] / 1000000).toFixed(1),
                            errorCode: cleanRecord[0][4],
                            errorDescription: Roborock.GET_ERROR_CODE_DESCRIPTION(cleanRecord[0][4]),
                            finishedFlag: (cleanRecord[0][5] === 1)
                        };
                    } catch (err) {
                        Logger.error(err);
                    }
