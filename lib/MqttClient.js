...
// requires are only needed for JSDoc / autocompletion
...
class MqttClient {
    ...
    /** @param statusData {import("./miio/Status")} */
                    ...
                    this.vacuum.getCleanSummary()
                        .then(res2 => {
                            ...
                            if (last_runs.length > 0) {
                                this.vacuum.getCleanRecord(last_runs[0])
                                    .then(data => {
                                        this.last_run_stats = {
