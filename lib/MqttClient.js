...
class MqttClient {
    ...
    constructor (options) {
        ...
        this.events.onMapUpdated(() => {
            if (this.provideMapData === true) {
                this.updateMapDataTopic(this.vacuum.robotState.map);
            }
        });

        this.events.onStatusUpdated((robotState) => {
            this.updateStatusTopic(robotState);
            this.updateAttributesTopicOnEvent(robotState);
        });
    ...
    updateMapDataTopic(map) {
        if (this.client && this.client.connected === true && map) {
            this.client.publish(this.topics.map_data, JSON.stringify(map), {retain: true, qos:this.qos});
        }
    }
    ...
    updateAttributesTopicOnEvent(robotState) {
        ...
        if (StatusStateAttribute) {
            ...
            if (StatusStateAttribute.flag !== attributes.StatusStateAttribute.FLAG.NONE) {
                this.last_state += " " + StatusStateAttribute.flag;
            }
        }
    ...
    updateStatusTopic(robotState) {
        ...
        if (this.client && this.client.connected === true) {
            ...
            if (StatusStateAttribute) {
                ...
                if (StatusStateAttribute.value === attributes.StatusStateAttribute.VALUE.ERROR) {
                    response.error = StatusStateAttribute.metaData.error_description;
                }

                if (StatusStateAttribute.flag !== attributes.StatusStateAttribute.FLAG.NONE) {
                    response.state += " " + StatusStateAttribute.flag;
                }
            }
