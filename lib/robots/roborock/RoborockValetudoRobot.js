...
class RoborockValetudoRobot extends MiioValetudoRobot {
    ...
    async pollState() {
        ...
        //Feel free to open a PR if you have a better idea how to handle this
        if (
            this.capabilities[capabilities.RoborockMapSegmentRenameCapability.TYPE] &&
            this.capabilities[capabilities.RoborockMapSegmentRenameCapability.TYPE].segmentNames === undefined
        ) {
    ...
    parseAndUpdateState(data) {
        ...
        if (data["water_box_mode"] !== undefined) {
            ...
            if (!matchingWaterGrade) {
                matchingWaterGrade = stateAttrs.IntensityStateAttribute.VALUE.CUSTOM;
            }

            this.state.upsertFirstMatchingAttribute(new stateAttrs.IntensityStateAttribute({
                type: stateAttrs.IntensityStateAttribute.TYPE.WATER_GRADE,
                value: matchingWaterGrade,
                customValue: matchingWaterGrade === stateAttrs.IntensityStateAttribute.VALUE.CUSTOM ? data["water_box_mode"] : undefined
            }));
        }
