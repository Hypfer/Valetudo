...
class Dreame1CVoicePackManagementCapability extends VoicePackManagementCapability {
     ...
     * @param {number} options.download_progress_piid
    ...
    constructor(options) {
        ...
        this.download_progress_piid = options.download_progress_piid;
        ...
        const res = await this.robot.sendCommand("get_properties", [
            {
                did: this.robot.deviceId,
                siid: this.siid,
                piid: this.download_progress_piid
            }
        ]);

        if (res?.length === 1) {
            if (res[0].code === 0) {
                let progress = res[0].value;

                if (progress < 100) {
                    statusOptions.type = ValetudoVoicePackOperationStatus.TYPE.DOWNLOADING;
                    statusOptions.progress = progress;
                } else if (progress === 100) {
                    statusOptions.type = ValetudoVoicePackOperationStatus.TYPE.IDLE;
                } else {
                    statusOptions.type = ValetudoVoicePackOperationStatus.TYPE.IDLE;
                    Logger.warn("Dreame1CVoicePackManagementCapability: Unhandled state", res);
                }
