...
class MiioValetudoRobot extends ValetudoRobot {
    ...
    constructor(options) {
        ...
        /**
         * this has to be "127.0.0.1" when running on roborock with the current iptables redirect approach
         * "0.0.0.0" will not work
         */
        this.dummycloudBindIp = this.implConfig && this.implConfig.dummycloudBindIp ? this.implConfig.dummycloudBindIp : "127.0.0.1";
        ...
        */
        ...
        this.mapUploadServer.listen(8079, "127.0.0.1", function() {
            Logger.info("Map Upload Server running on port " + 8087);
        });
    }
    ...
    get localSecret() {
        ...
        if (!localSecret) {
            ...
            try {
                line = fs.readFileSync(this.tokenFilePath);
            } catch (e) {
                Logger.debug("cannot read", this.tokenFilePath, e);
            }
            ...
            Logger.error("Unable to read token file. Local communication will fail");

            return Buffer.alloc(16);

        } else {
            return Buffer.from(localSecret, "hex");
        }
    }
