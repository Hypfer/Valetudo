...
class ViomiBasicControlCapability extends BasicControlCapability {
     ...
     */
    getVacuumMovementMode(operationMode) {
        if (operationMode === attributes.ViomiOperationMode.MOP) {
            // All the other movement modes turn on the vacuum motor. This mode is only available if the water-only
            // box is installed.
            return attributes.ViomiMovementMode.Y_NO_VACUUM;
        }

        const userMovementMode = this.robot.state.getFirstMatchingAttribute({
        ...
        if (userMovementMode === stateAttrs.PresetSelectionStateAttribute.MOVEMENT_MODE.AUTO) {
            switch (operationMode) {
                case attributes.ViomiOperationMode.MIXED:
                    return attributes.ViomiMovementMode.MODE_Y;
                case attributes.ViomiOperationMode.MOP:
                    return attributes.ViomiMovementMode.Y_NO_VACUUM;
                case attributes.ViomiOperationMode.VACUUM:
                    return attributes.ViomiMovementMode.MODE_S;
            }
        }

        switch (userMovementMode) {
            case stateAttrs.PresetSelectionStateAttribute.MOVEMENT_MODE.MODE_S:
                return attributes.ViomiMovementMode.MODE_S;
            case stateAttrs.PresetSelectionStateAttribute.MOVEMENT_MODE.MODE_Y:
                return attributes.ViomiMovementMode.MODE_Y;
            case stateAttrs.PresetSelectionStateAttribute.MOVEMENT_MODE.OUTLINES:
                return attributes.ViomiMovementMode.OUTLINES;
        }

        throw new Error("Bug! This should never be reached.");
    }
    ...
    async setModeWithSegments(operation, segmentIds) {
        ...
        const movementMode = this.getVacuumMovementMode(operationMode);
