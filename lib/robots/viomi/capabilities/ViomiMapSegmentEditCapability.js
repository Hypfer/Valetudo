...
class ViomiMapSegmentEditCapability extends MapSegmentEditCapability {
    ...
    async joinSegments(segmentA, segmentB) {
        if (this.robot.currentMapId === undefined) {
            throw new Error("Can't join segments if currentMapId is undefined");
        }

        await this.robot.sendCommand("arrange_room", {
            lang: this.lang,
            mapId: this.robot.currentMapId,
    ...
    async splitSegment(segment, pA, pB) {
        if (this.robot.currentMapId === undefined) {
            throw new Error("Can't split segment if currentMapId is undefined");
        }
