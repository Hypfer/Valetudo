...
const Tools = {
    ...
    DRAW_MAP_PNG: function (options, callback) {
        const COLORS = {
            ...
            nothing: Jimp.rgbaToInt(0,0,0,0),
        ...
        new Jimp(DIMENSIONS.width, DIMENSIONS.height, function (err, image) {
            if (!err) {
                ...
                options.mapDTO.parsedData.image.pixels.forEach(function drawPixel(px) {
                    let color;
                    const coordinates = {};

                    if (px[2] === 0) { //TODO: is there obstacle_weak?
                    ...
                    try {
                        image.setPixelColor(color, px[0], px[1]); //TODO: was out of bounds!
                    } catch(e) {
                        console.error(e);
                        console.log(px, DIMENSIONS, options.mapDTO);
                    }
                });
