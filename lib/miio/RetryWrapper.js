...
class RetryWrapper {
    ...
    sendMessage(method, args = [], options = {}, callback) {
        if (!this.miioSocket.stamp.isValid()) {
            return this.handshake().then(() => this.sendMessage(method, args, options, callback));
        }
        ...
        const onError = (err) => {
            ...
            if (options.retries > 10000) {
                callback(new MiioSocket.TimeoutError(msg), null);
            ...
            this.handshake(true).then((id) => {
                this.miioSocket
                    ...
                    .then((res) => callback(null, res), onError);
