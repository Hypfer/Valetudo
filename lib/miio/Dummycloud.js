...
const Dummycloud = function (options) {
    ...
    this.socket.on("listening", () => {
        log.info("Dummycloud is spoofing " + this.spoofedIP + ":8053 on " + this.bindIP +":8053");
    });
    ...
    this.socket.on("message", (incomingMsg, rinfo) => {
        ...
        if (decodedResponse.msg === null) {
            if(decodedResponse.stamp === 0) { //Initial TimeSync Packet
                ...
                log.info("Robot connected"); //TODO: Some kind of state which is available to the web
            ...
            //Also, it doesn't care about using the same message id many times
                ...
                switch(decodedResponse.msg.id) {
                    case Dummycloud.SERVER_REQUESTS.MAP:
                        if(Array.isArray(decodedResponse.msg.result) && decodedResponse.msg.result.length === 1) {
                            let timeout = [4,5,6,7,11,15,16,17].indexOf(this.connectedRobot.status.state) !== -1 ? 200 : 60000;
                ...
                log.info("Unknown cloud message received:", JSON.stringify(decodedResponse.msg));
            }
        }
        ...
        if (response) {
            ...
            log.info("No response for message:", JSON.stringify(decodedResponse.msg));
        }
    });
};
