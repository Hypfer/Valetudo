...
const Dummycloud = function (options) {
    ...
    this.socket.on("listening", () => {
        Logger.info("Dummycloud is spoofing " + this.spoofedIP + ":8053 on " + this.bindIP +":8053");
    });
    ...
    this.socket.on("message", (incomingMsg, rinfo) => {
        ...
        if (decodedResponse.msg === null) {
            if(decodedResponse.stamp === 0) { //Initial TimeSync Packet
                ...
                Logger.info("Robot connected"); //TODO: Some kind of state which is available to the web
            ...
            //Also, it doesn't care about using the same message id many times
                ...
                Logger.info("Unknown cloud message received:", JSON.stringify(decodedResponse.msg));
            }
        }
        ...
        if (response) {
            ...
            Logger.info("No response for message:", JSON.stringify(decodedResponse.msg));
        }
    });
...
Dummycloud.prototype.shutdown = function() {
    return new Promise((resolve, reject) => {
        Logger.debug("Dummycloud shutdown in progress...");
        ...
        this.socket.close(() => {
            Logger.debug("Dummycloud shutdown done");
            resolve();
        });
    });
};
