...
Vacuum.prototype.startCleaningZone = function(zones, callback) {
    if(Array.isArray(zones) && zones.length <= 5) {
        for(var i=0; i<zones.length; i++){
            ...
            zones[i][1] = Tools.DIMENSION_MM - zones[i][1];
            zones[i][3] = Tools.DIMENSION_MM - zones[i][3];
...
Vacuum.prototype.update = function(url, md5, callback) {
    this.sendMessage("miIO.ota", {
        "mode": "normal",
        "install": "1",
        "app_url": url,
        "file_md5": md5,
...
/**
 * Returns update status like "idle", "downloading", "installing" or "failed"
 */
Vacuum.prototype.getUpdateStatus = function(callback) {
    this.sendMessage("miIO.get_ota_state", [], {}, Vacuum.GET_ARRAY_HANDLER(callback));
};

/**
 * Returns update progress from 0 to 100
 */
Vacuum.prototype.getUpdateProgress = function(callback) {
    this.sendMessage("miIO.get_ota_progress", [], {}, Vacuum.GET_ARRAY_HANDLER(callback));
};
