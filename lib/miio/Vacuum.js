...
const GLOBALS = require("../res/global_vars");
...
const Vacuum = function(options) {
    ...
    this.events.on("miio.status", (statusData) => {
        if (this.zoneCleaningStatus && statusData.battery !== undefined){
            ...
            if (state != GLOBALS.TEXT_STATES.ZONED_CLEANING && 
                state != GLOBALS.TEXT_STATES.PAUSED && 
                state != GLOBALS.TEXT_STATES.RETURNING_HOME && 
                state != GLOBALS.TEXT_STATES.DOCKING){ //Let's null zone cleaning after vacuum is docked and ready to have new commands.
                this.zoneCleaningStatus = null;
            }
        }
    })
...
Vacuum.GET_ERROR_CODE_DESCRIPTION = function(errorCodeId) {
    if (GLOBALS.ERROR_CODES.hasOwnProperty(errorCodeId)) {
        return GLOBALS.ERROR_CODES[errorCodeId];
    } else {
        return GLOBALS.ERROR_CODES.UNKNOWN;
    }
};

Vacuum.GET_STATE_CODE_DESCRIPTION = function(stateCodeId) {
    if (GLOBALS.STATES_TO_TEXT.hasOwnProperty(stateCodeId)) {
        return GLOBALS.STATES_TO_TEXT[stateCodeId];
    } else {
        return GLOBALS.STATES.UNKNOWN;
    }
};
