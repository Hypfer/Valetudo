...
Vacuum.GET_ERROR_CODE_DESCRIPTION = function(errorCodeId) {
    if (Vacuum.ERROR_CODES[errorCodeId] !== undefined) {
...
Vacuum.GET_STATE_CODE_DESCRIPTION = function(stateCodeId) {
    if (Vacuum.STATES[stateCodeId] !== undefined) {
...
Vacuum.prototype.shutdown = function() {
    return new Promise((resolve, reject) => {
        Logger.debug("Vacuum shutdown in progress...");
        ...
        this.socket.close(() => {
            Logger.debug("Vacuum shutdown done");
            resolve();
        });
    });
};
