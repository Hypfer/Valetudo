...
class DreameMapParser {
    ...
    static PARSE(buf, type) {
        ...
        return new Map.ValetudoMap({
            ...
            metaData: {
                dreame: {
                    offsets: {
                        left: parsedHeader.left,
                        top: parsedHeader.top
                    }
                }
            }
        });
    ...
    static PARSE_HEADER(buf) {
        ...
        //TODO: instead of doing this, there should simply be no charger and robot at all
        if (buf.readInt16LE(5) === HALF_INT16_UPPER_HALF && buf.readInt16LE(7) === HALF_INT16_UPPER_HALF) {
            parsedHeader.robot_position = {
                x: 0,
                y: 0,
                angle: 0
            };
    ...
    static PARSE_IMAGE(parsedHeader, activeSegmentIds, buf) {
        const floorPixels = [];
        const wallPixels = [];
        const segments = {};
        ...
        for (let i = 0; i < parsedHeader.height; i++) {
            for (let j = 0; j < parsedHeader.width; j++) {
                /**
                 * A Pixel is one byte consisting of
                 *      000000               00
