...
class DreameMapParser {
    ...
    static PARSE(buf, type) {
        ...
        if (parsedHeader.robot_position.valid === true) {
            entities.push(
                new Map.PointMapEntity({
                    ...
                    metaData: {
                        angle: parsedHeader.robot_position.angle
                    },
        ...
        if (parsedHeader.charger_position.valid === true) {
            entities.push(
                new Map.PointMapEntity({
                    ...
                    metaData: {
                        angle: parsedHeader.charger_position.angle
                    },
        ...
        if (buf.length >= HEADER_SIZE + parsedHeader.width * parsedHeader.height) {
            ...
            layers.push(...DreameMapParser.PARSE_IMAGE(parsedHeader, activeSegmentIds, imageData, type));
                    ...
                    rismResult.layers.forEach(l => {
                        if (l.metaData.segmentId !== undefined) {
                            if (layers.findIndex(eL => eL.metaData.segmentId === l.metaData.segmentId) === -1) {
                                ...
                                //TODO: should we drop the duplicates?
                            }
                        } else {
            ...
            if (additionalData.tr) {
                entities.push(
                    new Map.PathMapEntity({
                        points: DreameMapParser.PARSE_PATH(parsedHeader, additionalData.tr, additionalData.l2r === 1),
    ...
    static PARSE_IMAGE(parsedHeader, activeSegmentIds, buf, type) {
        const floorPixels = [];
        const wallPixels = [];
        const segments = {};
