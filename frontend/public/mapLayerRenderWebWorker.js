...
/*
    Unfortunately, we have to duplicate this rendering code for it to be available in a webworker,
    because as of now 2021-10-06, this putting the worker into the public dir hack is the only
    way to use a webworker with create-react-app
...
self.addEventListener( "message", ( evt ) => {
    ...
    evt.data.mapLayers.sort((a,b) => {
        ...
        for (let i = 0; i < layer.pixels.length; i = i + 2) {
            const imgDataOffset = (layer.pixels[i] + layer.pixels[i+1] * evt.data.width) * 4;
