...
class MideaE20ValetudoRobot extends MideaValetudoRobot {
    constructor(options) {
        super(
            Object.assign(
                {},
    ...
    parseAndUpdateState(data) {
        ...
        if (_data.elevator_switch !== undefined) {
            let matchingOperationMode = Object.keys(OPERATION_MODES).find(key => {
                return OPERATION_MODES[key] === _data.elevator_switch;
            });
            ...
            this.state.upsertFirstMatchingAttribute(new stateAttrs.PresetSelectionStateAttribute({
                ...
                metaData: {
                    rawValue: _data.elevator_switch
                },
            }));
        }

        if (_data.work_status !== undefined) {
            if (_data.work_status === 16) {
            ...
            if (_data.work_status === 12 && _data.battery_percent !== 100) {

                _data.work_status = 112;
            }
        }

        super.parseAndUpdateState(_data);
    }
...
const STATUS_MAP = Object.freeze({
    ...
    // TODO: Apparently, 171 exists?
});




module.exports = MideaE20ValetudoRobot;
